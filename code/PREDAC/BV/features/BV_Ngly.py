...
df_virus = pd.read_csv(r'../../../data/BV_sample.csv',index_col=0)

with open(r'../../../data/BVHA1_Ngly.txt') as f:
    lines = f.readlines()
...
# print(vir_dict['3870'])
# Define a function to calculate the number of different glycosylation sites between two viruses
def gly_diff(vir1,vir2):
    list1 = vir_dict[vir1] # List of glycosylation sites for virus 1
    list2 = vir_dict[vir2] # List of glycosylation sites for virus 2
    num_diff = len(list1) + len(list2) - (2*len(set(list1)&set(list2))) # Calculate the number of different glycosylation sites between two viruses
    return num_diff

# Load model data
data = pd.read_csv(r'./result/BV_model.csv',index_col=0)
print(data.columns)
# Load all sequences and IDs
df_seq = pd.read_csv(r'../../../data/BV_sample.csv',index_col=0)

# Rename columns for easier matching
df_seq.rename(columns={'seq':'virus1_seq','num':'virus1_num'},inplace=True)
print(df_seq)
# Match IDs for virus 1
...
print(df_seq)
# Match IDs for virus 2
df = pd.merge(df,df_seq,on='virus2_seq')
print(df.shape[0])
df.reset_index(inplace=True,drop=True)

print(df.columns)
# Note: convert data type, the original type is numeric and needs to be converted to string
