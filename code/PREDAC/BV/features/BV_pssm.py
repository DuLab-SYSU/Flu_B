...
df = pd.read_csv('../../../dataBV_ALL_SCAN.csv')

# Split the original file into three dataframes according to chains A, C, and E
df_A = df.loc[df['Chain'] == 'A'].copy()
df_C = df.loc[df['Chain'] == 'C'].copy()
df_E = df.loc[df['Chain'] == 'E'].copy()

# Reset index
# Position 163 has three insertions; adjust residue numbering to match the original PDB file for correct marking in the structure
df_A.reset_index(drop=True, inplace=True)
df_C.reset_index(drop=True, inplace=True)
df_E.reset_index(drop=True, inplace=True)

# Reassign residue numbering
# Position 163 has three insertions; adjust residue numbering to match the original PDB file for correct marking in the structure
# Note: for order, use continue or if...elif instead of if...if
...
# Read the residue exposure file into a dataframe
# Identify exposed amino acid sites — based on freesasa software
with open(r'../../../data/BV_RSA_freesasa.txt') as f:
    lines = f.readlines()

# Convert RSA, SASA, etc. calculated by freesasa into dataframe format
...
# Divide into exposed and non-exposed sites by 20% threshold,
# i.e., sites with RSA below the 20th percentile are non-exposed, otherwise exposed
...
# Integrate RSA and other exposure data into antigen site dataframes
...
# Sort by predicted binding probability
...
# Select the top 150 sites for each chain, and keep only exposed sites
...
# Adjust index to match real residue positions
...
estimator = KMeans(n_clusters=5,random_state=2022)   # Construct clustering model
estimator.fit(df_BV.loc[list_index,['x_coord','y_coord','z_coord']])      # Perform clustering
label_pred = estimator.labels_     # Get clustering labels
df_BV.loc[list_index,'label'] = label_pred

# Divide the epitope regions into lists
...
# Column indices used for locating the PSSM matrix
...
def pssm_diff(list_index, seq1, seq2):
    '''Calculate PSSM score sum based on epitopes'''
...
print(list_epi4)
print(list_epi5)
# Import HI dataset (median values)
data = pd.read_csv(r'./BV_model.csv',index_col=0)

# Construct BV epitope features
