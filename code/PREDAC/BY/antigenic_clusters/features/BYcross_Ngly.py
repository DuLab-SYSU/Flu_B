# -*- coding: utf-8 -*-
# @Time    : 2023/12/25 下午9:14
# @Author  : Hanwenjie
# @project : code
# @File    : BYcross_Ngly.py
# @IDE     : PyCharm
# @REMARKS : 说明文字
...
df_virus = pd.read_csv(r'../../../../data/sequence/features_related/BY_sample.csv',index_col=0)

with open(r'../../../../data/sequence/features_related/BYHA1_Ngly.txt') as f:
	lines = f.readlines()
...
def gly_diff(vir1,vir2):
    list1 = vir_dict[vir1] 
    list2 = vir_dict[vir2] 
    num_diff = len(list1) + len(list2) - (2*len(set(list1)&set(list2))) #计算两个病毒不同的糖基化位点数目
    return num_diff
