...
def observe_nb2(name, latent, det_prob, dispersion, obs=None):
    ...
    mask = True
    if obs is not None:
        mask = jnp.isfinite(obs) & (obs >= 0.0)
        obs = jnp.where(mask, obs, 0.0)
    ...
    det_prob = jnp.broadcast_to(det_prob, latent.shape)
...
def param_estimate(R0_v_value, R0_y_value,
    ...
    df_params_orign.to_csv(f'./result/fluB/params/orign/params_group{file_name}.csv', index=False)
    ...
    # export data
    # df_params.to_csv(f'./result/fluB/params/az/params_az_group{file_name}.csv', index=False)
    df_posterior.to_csv(f'./result/fluB/posterior/params_group{file_name}.csv', index=False)

    # plot trace
    az.plot_trace(az_trace, var_names=params_orign)
    plt.tight_layout()
    plt.savefig(f'./result/fluB/figure/trace/params_group{file_name}.png')

    # plot posterior
    az.plot_posterior(az_trace, var_names=params_orign, grid=(5,5))
    plt.tight_layout()
    plt.savefig(f'./result/fluB/figure/posterior/params_group{file_name}.png')
    ...
    plt.savefig(f'./result/fluB/figure/fitting/params_group{file_name}.png')
...
# param_estimate(R0_v_value=1.6, R0_y_value=1.5,  
#                w_v_value=0.5, w_y_value=0.5, 
#                cv_value=0.5, cy_value=0.5, 
#                tv_0_value=18, ty_0_value=18,
#                r_value=0.5,
#                S_rate_ini_value=0.5, 
#                Rcv_rate_ini_value=0.05, Rcy_rate_ini_value=0.05, 
#                Rv_rate_ini_value=0.2, Ry_rate_ini_value=0.2,
#                R_rate_ini_value=0.1,
#                file_name=1)
