...
def process_extinction_time(args):
    cv, cy = args
    df = pd.read_csv(f"./result/simmulation/data_t/data_simmulation_orign_cv{cv:.2f}_cy{cy:.2f}.csv", index_col='time_plot')
...
cv_values = np.arange(0.0, 30.1, 0.1)
cy_values = np.arange(0.0, 30.1, 0.1)
...
df_by_time.to_csv(r"./result/simmulation/by_extinction_time_changeT.csv")
...
df_by_size.to_csv(r"./result/simmulation/by_extinction_size_changeT.csv")
