...
def dz_dt(z, t, tv_c, ty_c, w_v_old, w_v_new, w_y_old, w_y_new, R0_v, R0_y, D, ksi, Lv, Ly, omega, cv, cy, immue_esp_pct_v1, immue_esp_pct_v2, immue_esp_pct_v3, immue_esp_pct_y1, immue_esp_pct_y2, npi_intensity, npi_start, npi_end, tv_0, ty_0, T, immue_esp_vt1, immue_esp_vt2, immue_esp_vt3, immue_esp_yt1, immue_esp_yt2, Pv, Py):
    ...
    # cross protection
    w_v = jnp.where((t >= tv_c), w_v_new, w_y_old)
    w_y = jnp.where((t >= ty_c), w_y_new, w_y_old)
...
def generate_data(S_rate_ini, Iv_rate_ini, Iy_rate_ini, Rcv_rate_ini, Rcy_rate_ini,
                  ...
                  R0_v, R0_y, w_v_old, w_v_new, w_y_old, w_y_new, cv, cy, tv_0, ty_0, tv_c, ty_c, immue_esp_pct_v1, immue_esp_pct_v2, immue_esp_pct_v3, 
...
by_esp = 0.00
npi = 0.10
for cv_value in tqdm(np.arange(0.0, 1.01, 0.01)):
    for cy_value in np.arange(0.0, 1.01, 0.01):
        ss = generate_data(S_rate_ini = dict_params['S_rate_ini'], Iv_rate_ini = dict_params['Iv_rate_ini'], 
                           ...
                           cv = dict_params['cv'], cy = dict_params['cy'], 
                           w_v_old = dict_params['w_v'], w_v_new = cv_value, 
                           w_y_old = dict_params['w_y'], w_y_new = cy_value, 
        ...
        df_result.to_csv(f"./result/simmulation/data_w/data_simmulation_orign_cv{cv_value:.2f}_cy{cy_value:.2f}.csv") 
