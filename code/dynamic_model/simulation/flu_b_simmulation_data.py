...
def dz_dt(z, t, w_v, w_y, R0_v, R0_y, D, ksi, Lv, Ly, omega, cv, cy, immue_esp_pct_v1, immue_esp_pct_v2, immue_esp_pct_v3, immue_esp_pct_y1, immue_esp_pct_y2, npi_intensity, npi_start, npi_end, tv_0, ty_0, T, immue_esp_vt1, immue_esp_vt2, immue_esp_vt3, immue_esp_yt1, immue_esp_yt2, Pv, Py):
    ...
    # import_v = jnp.where((t >= npi_start) & (t <= npi_end), 
    #                      jnp.where(npi_intensity == 0, 2., 
    #                                jnp.where(npi_intensity <= 0.1, 1., 0.)), 
    #                      2.)

    # import_y = jnp.where((t >= npi_start) & (t <= npi_end), 
    #                      jnp.where(npi_intensity == 0, 2., 
    #                                jnp.where(npi_intensity <= 0.1, 1., 0.)), 
    #                      2.)
...
for bv_esp in tqdm(np.arange(0.0, 0.21, 0.01)):
    for by_esp in np.arange(0.0, 0.101, 0.001):
        for npi in np.arange(0.0, 1.01, 0.01):
            ss = generate_data(S_rate_ini = dict_params['S_rate_ini'], Iv_rate_ini = dict_params['Iv_rate_ini'], 
                               ...
                               cv = dict_params['cv'], cy = dict_params['cy'], tv_0 = dict_params['tv_0'], ty_0 = dict_params['ty_0'], 
            ...
            df_result[f'S_y{by_esp:.3f}_n{npi:.2f}'] = S
            df_result[f'Iv_y{by_esp:.3f}_n{npi:.2f}'] = Iv
            df_result[f'Iy_y{by_esp:.3f}_n{npi:.2f}'] = Iy
            df_result[f'Rcv_y{by_esp:.3f}_n{npi:.2f}'] = Rcv
            df_result[f'Rcy_y{by_esp:.3f}_n{npi:.2f}'] = Rcy
            df_result[f'Rv_y{by_esp:.3f}_n{npi:.2f}'] = Rv
            df_result[f'Ry_y{by_esp:.3f}_n{npi:.2f}'] = Ry
            df_result[f'Jv_y{by_esp:.3f}_n{npi:.2f}'] = Jv
            df_result[f'Jy_y{by_esp:.3f}_n{npi:.2f}'] = Jy
            df_result[f'R_y{by_esp:.3f}_n{npi:.2f}'] = R
            df_result[f'CV_y{by_esp:.3f}_n{npi:.2f}'] = CV
            df_result[f'CY_y{by_esp:.3f}_n{npi:.2f}'] = CY
            df_result[f'BV_y{by_esp:.3f}_n{npi:.2f}'] = dict_params['det_prob_v'] * df_result[f'CV_y{by_esp:.3f}_n{npi:.2f}'].diff()
            df_result[f'BY_y{by_esp:.3f}_n{npi:.2f}'] = dict_params['det_prob_y'] * df_result[f'CY_y{by_esp:.3f}_n{npi:.2f}'].diff()
            ...
            df_result.to_csv(f"./result/simmulation/data/data_simmulation_orign_v{bv_esp:.2f}_y{by_esp:.3f}_n{npi:.2f}.csv")
